...
AVAILABLE_MODELS = {
    ...
    "U2Net": {
    "type": "u2net",
    "repo_id": "",  # optional, only used if downloading
    "files": {
        "model.py": "model.py",
        "u2net.pth": "u2net.pth"
        },
    ...
    def load_model(self, model_name):
        if self.current_model_version != model_name:
            ...
            try:
                cache_dir = self.get_cache_dir(model_name)
                model_path = os.path.join(cache_dir, "model.py")
                module_name = f"custom_u2net_model_{hash(model_path)}"
                
                spec = importlib.util.spec_from_file_location(module_name, model_path)
                u2net_module = importlib.util.module_from_spec(spec)
                sys.modules[module_name] = u2net_module
                spec.loader.exec_module(u2net_module)
                
                model_weights_path = os.path.join(cache_dir, "u2net.pth")
                self.model = u2net_module.U2NET()
                ...
                handle_model_error(f"Error loading U2Net model: {str(e)}")
        ...
        try:
            ...
            for i in range(0, len(images), batch_size):
                ...
                for img in batch_images:
                    ...
                    new_h = int(params["process_res"] * aspect_ratio)
                    resized_image = orig_image.resize((new_w, new_h), Image.LANCZOS)
                    batch_pil_images.append(resized_image)
                
                with torch.no_grad():
                    try:
                        # Prepare input tensors for U2Net
                        input_tensors = []
                        for pil_img in batch_pil_images:
                        ...
                        # U2Net returns multiple outputs, we use the main output
                        outputs = self.model(input_batch)
                        if isinstance(outputs, tuple):
                            masks = outputs[0].cpu()  # First output is already sigmoided
                        ...
                        for j, (mask, (orig_w, orig_h)) in enumerate(zip(masks, original_sizes)):
                            mask = mask.squeeze()
                            ...
                            mask_pil = tensor2pil(mask_resized)
                            all_masks.append(mask_pil)
                            
                    except Exception as e:
                        handle_model_error(f"Error in U2Net inference: {str(e)}")
