...
AVAILABLE_MODELS = {
    ...
    "U2Net": {
        "type": "u2net",
        "repo_id": "your_u2net_repo_id",  # Replace with actual repo ID
        "files": {
            "u2net_arch.py": "u2net_arch.py",
            "model.py": "model.py",
            "u2net.pth": "u2net.pth"
        },
        "cache_dir": "U2Net"
...
class U2NetModel(BaseModelLoader):
    def __init__(self):
        super().__init__()
    ...
    def load_model(self, model_name):
        if self.current_model_version != model_name:
            ...
            # First load the architecture module
            arch_path = os.path.join(cache_dir, "u2net_arch.py")
            arch_module_name = f"custom_u2net_arch_{hash(arch_path)}"
            arch_spec = importlib.util.spec_from_file_location(arch_module_name, arch_path)
            arch_module = importlib.util.module_from_spec(arch_spec)
            sys.modules[arch_module_name] = arch_module
            arch_spec.loader.exec_module(arch_module)
            
            # Now load the model module
            model_path = os.path.join(cache_dir, "model.py")
            model_module_name = f"custom_u2net_model_{hash(model_path)}"
            model_spec = importlib.util.spec_from_file_location(model_module_name, model_path)
            model_module = importlib.util.module_from_spec(model_spec)
            sys.modules[model_module_name] = model_module
            model_spec.loader.exec_module(model_module)
            
            # Get the U2NET class from the architecture module
            U2NET = arch_module.U2NET
            
            # Initialize the model
            self.model = U2NET()
            
            # Load weights
            weights_path = os.path.join(cache_dir, "u2net.pth")
    ...
    def process_image(self, image, model_name, params):
        try:
            ...
            # Preprocess the image for U2Net
            ...
            with torch.no_grad():
                ...
                # Normalize and threshold the prediction
                pred = d1[:, 0, :, :]
                pred = (pred - pred.min()) / (pred.max() - pred.min())
                pred = pred.squeeze()
                pred = F.interpolate(pred.unsqueeze(0).unsqueeze(0), size=(h, w), mode='bilinear').squeeze()
                
                # Convert to PIL Image
                mask = Image.fromarray((pred.cpu().numpy() * 255).astype(np.uint8))
                
            return mask
        except Exception as e:
            handle_model_error(f"Error in U2Net processing: {str(e)}")
...
class RMBG:
    def __init__(self):
        self.models = {
            ...
            "BEN": BENModel(),
            "BEN2": BEN2Model(),
            "U2Net": U2NetModel()  # Added U2Net support
        }
    @classmethod
    def INPUT_TYPES(s):
        tooltips = {
    ...
    def process_image(self, image, model, **params):
        try:
            ...
            if model_type in ("rmbg", "ben2", "u2net"):
